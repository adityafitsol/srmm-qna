#!/usr/bin/env python3
"""
SRMM Questionnaire Scraper — Agent 2
=====================================
Reads companies_list.txt, deduplicates, searches for each company's BRSR
FY 2022-23 PDF via Gemini Google Search, scores all SRMM v2.0 indicators,
and saves results to:
  • srmm-qna-agent2.json
  • srmm-qna-agent2.xlsx

Run:    python3 srmm_scraper_agent2.py
Resume: re-run same command — already-done companies are skipped.
"""

import json, os, sys, time, logging, traceback, re, signal
import requests
...
COMPANIES_TXT     = BASE_DIR / "companies_list.txt"
OUTPUT_JSON       = BASE_DIR / "srmm-qna-agent2.json"
OUTPUT_EXCEL      = BASE_DIR / "srmm-qna-agent2.xlsx"
PROGRESS_FILE     = BASE_DIR / ".srmm_progress_agent2.json"
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(BASE_DIR / "srmm_scraper_agent2.log"),
...
# ═══════════════════════════════════════════════════════════════════════════════
# LOAD COMPANIES FROM TXT  (deduplicate, preserve order)
# ═══════════════════════════════════════════════════════════════════════════════
def load_companies_txt(path: Path) -> list[dict]:
    seen   = set()
    result = []
    with open(path, encoding="utf-8") as f:
        for idx, line in enumerate(f, 1):
            ...
            if key in seen:
                continue
            seen.add(key)
            result.append({"sno": len(result) + 1, "company": name, "brsr_link": None})
    return result
...
def build_prompt(company_name: str) -> str:
    ...
    return f"""You are an expert BRSR analyst specializing in SRMM v2.0 framework by ICAI.
...
def analyze_with_gemini(pdf_bytes: bytes, company_name: str) -> dict | None:
    ...
    cfg = GenerationConfig(temperature=0.1, response_mime_type="application/json")
    for attempt in range(1, RETRY_ATTEMPTS + 1):
        try:
            ...
            if "indicators" not in parsed:
                raise ValueError("Missing 'indicators' key")
...
def calculate_scores(indicators: dict) -> dict:
    ...
    principles = {f"P{i}": {"essential": 0, "leadership": 0, "total": 0,
                  ...
                  for i in range(1, 10)}
    for q in SRMM_SCHEMA:
        qid  = q["id"]; sec = q["sec"]; cat = q["cat"]
        pri  = q.get("pri", ""); mmax = q["max"]
        raw  = indicators.get(qid, {})
        sc   = min(int(raw.get("score", 0)), mmax)
        if sec == "A":
            section_a += sc
        elif sec == "B":
            section_b += sc
    ...
    return {
        "section_a": section_a, "section_a_max": 18,
        "section_b": section_b, "section_b_max": 24,
        "principles": principles,
        "essential_total": essential_total, "leadership_total": leadership_total,
        "grand_total": grand_total, "max_score": GRAND_MAX,
        "percentage": pct, "maturity_level": level, "maturity_stage": stage,
    }
...
def process_company(company: dict) -> dict:
    ...
    base = {"sno": sno, "company": name, "brsr_link": None, "analyzed_at": ts}

    log.info(f"[{sno:4d}] {name}  →  searching BRSR FY 2022-23 PDF …")
    ...
    if pdf is None:
        log.warning(f"[{sno:4d}] {name}  →  PDF not found, skipping")
...
# ═══════════════════════════════════════════════════════════════════════════════
LEVEL_COLORS = {
    "Level 1": "FF4C4C", "Level 2": "FFA500",
    "Level 3": "FFD700", "Level 4": "4CAF50", "N/A": "CCCCCC",
}
HDR_FILL  = PatternFill("solid", fgColor="1F4E79")
HDR_FONT  = openpyxl.styles.Font(bold=True, color="FFFFFF", size=10)
BODY_FONT = openpyxl.styles.Font(size=9)
...
def save_excel(all_results: list):
    ...
    hdrs = ["S.No","Company","Status","Sec A","Sec B",
    ...
    for ri, r in enumerate(all_results, 2):
        sc = r.get("scores") or {}; pr = sc.get("principles", {})
        ...
        row_data = [
            ...
            sc.get("percentage",""), level, sc.get("maturity_stage",""),
        ]
    ...
    ws2.row_dimensions[1].height = 40; ws2.freeze_panes = "D2"
    for ri, r in enumerate(all_results, 2):
        ans = r.get("answers") or {}
        row_data = [r["sno"], r["company"], r["status"]] + [ans.get(q,{}).get("score","") for q in ind_ids]
        for ci, val in enumerate(row_data, 1):
            ws2.cell(row=ri, column=ci, value=val).font = BODY_FONT

    # Sheet 3: Answers
    ws3 = wb.create_sheet("Answers")
    for ci, h in enumerate(["S.No","Company","Indicator ID","Parameter","Score","Max","Reason","Evidence"], 1):
        _hdr(ws3, 1, ci, h, width=14)
    ws3.column_dimensions["B"].width = 40; ws3.column_dimensions["D"].width = 45
    ws3.column_dimensions["G"].width = 55; ws3.column_dimensions["H"].width = 45
    ws3.freeze_panes = "A2"
    row_idx = 2
...
def main():
    log.info("══════════════════════════════════════════════════")
    log.info("  SRMM Scraper Agent 2  —  companies_list.txt    ")
    log.info("══════════════════════════════════════════════════")

    companies = load_companies_txt(COMPANIES_TXT)
    log.info(f"Loaded {len(companies)} unique companies from {COMPANIES_TXT}")
    ...
    def _flush(done, companies):
        ...
        out = {
            "metadata": {
                ...
                "no_link":    sum(1 for v in done.values() if v.get("status") == "pdf_not_found"),
                "failed":     sum(1 for v in done.values() if v.get("status") in ("gemini_failed","error")),
                "model": MODEL_NAME,
            },
            "companies": all_res,
        }
        ...
        try:
            for fut in as_completed(futures):
                ...
                try:
                    ...
                    log.error(f"Error sno={c['sno']}: {e}\n{traceback.format_exc()}")
                    result = {
                        "sno": c["sno"], "company": c["company"], "brsr_link": None,
                        "status": "error", "error": str(e), "answers": {}, "scores": None,
                ...
                if _shutdown_requested.is_set():
                    log.warning("Shutdown — cancelling remaining …")
    ...
    if _shutdown_requested.is_set():
        ...
        log.info("All done. Writing final outputs …")

    all_results = [done.get(str(c["sno"]), {
        "sno": c["sno"], "company": c["company"], "brsr_link": None,
        "status": "not_processed", "answers": {}, "scores": None,
    }) for c in companies]
    ...
    final_out = {
        "metadata": {
            ...
            "pdf_not_found": sum(1 for r in all_results if r.get("status") == "pdf_not_found"),
            "failed":     sum(1 for r in all_results if r.get("status") in ("gemini_failed","error")),
            "model": MODEL_NAME, "srmm_version": "2.0", "max_possible_score": GRAND_MAX,
        },
    ...
    log.info(f"  Done!  Completed: {final_out['metadata']['completed']}  "
             f"Not found: {final_out['metadata']['pdf_not_found']}  "
